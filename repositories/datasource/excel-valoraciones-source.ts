...
export class ExcelValoracionesDataSource implements ValoracionesDataSource {
  ...
  private readonly cache: CacheArchivo<Map<string, ValoracionConferencista>>;
  ...
  getValoracion(slug: string): ValoracionConferencista | null {
    return this.cache.obtener().get(slug) ?? null;
  }

  private parsear(): Map<string, ValoracionConferencista> {
    // El libro se abre una sola vez para todas las hojas del mapeo.
    const libro = leerLibro(this.ruta);
    const resultado = new Map<string, ValoracionConferencista>();

    for (const [slug, fuente] of Object.entries(MAPEO)) {
      const valoracion = this.leerHoja(libro, fuente);
  ...
  private leerHoja(libro: XLSX.WorkBook, fuente: FuenteValoracion): ValoracionConferencista | null {
    const filas = hojaAMatriz(libro, fuente.hoja);
