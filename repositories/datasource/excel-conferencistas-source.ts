 ...
 * intermedio y con caché por archivo, igual que el resto de los orígenes (ver
 * CacheArchivo).
 */
export class ExcelConferencistasDataSource implements ConferencistasDataSource {
  private readonly ruta: string;
  private readonly cache: CacheArchivo<ConferenciaCard[]>;
  ...
  private parsear(): ConferenciaCard[] {
    const filas = hojaAMatriz(leerLibro(this.ruta), "Base de Datos");
    if (!filas) return [];
